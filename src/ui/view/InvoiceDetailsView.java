...
public class InvoiceDetailsView extends View implements ActionListener{
	...
	public InvoiceDetailsView(Invoice invoice) {
		super("Invoice "+invoice.getIdInvoice()+" Details");
		...
		prodsOrder = shopFacade.getAllProductInOrder(invoice.getIdOrder());
		setProducts();
    }
	...
	public void setProducts(){
		...
		for(int i = 0 ; i < prodsOrder.size() ; i++) {
			...
			listPanel.add(makeProductCell(prod.getName(),prodO.getQuantity(),prod.getPrice(),i));
			total = total + prodO.getQuantity()*prod.getPrice();
		}
